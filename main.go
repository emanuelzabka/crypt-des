...
func generateRoundKeys() (result [][]byte) {
	block := make([]byte, 8)
	transformedKey := key64To56(block)
	transformedKey[0] = 0x80
	transformedKey[len(transformedKey)-1] = 0x01;
	result = make([][]byte, 16)
	for i := 0; i < 16; i++ {
